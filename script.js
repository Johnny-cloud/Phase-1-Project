document.addEventListener("DOMContentLoaded", () => {
    ...
    function searchArtist(artistName){
        ...
        fetch(`https://spotify23.p.rapidapi.com/search/?q=${artistName}&type=multi&offset=0&limit=10&numberOfTopResults=5`, options)
            ...
            .then(response => {
                ...
                response.albums.items.forEach(item => createSearchCard2(item));
                document.querySelector("#searched-term > h1").textContent = "Results";
            })
            .catch(err => console.error(err));
    ...
    function createSearchCard2(track) {
        ...
        titleP.textContent = track.data.artists.items[0].profile.name;
        img.src = track.data.coverArt.sources[0].url;
        artistP.textContent = track.data.name;
        img.className = "artist-img";
